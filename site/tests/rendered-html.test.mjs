...
test("server-renders the current usAIge release", async () => {
  ...
  assert.match(html, /usAIge-0\.1\.12-alpha\.dmg/);
  assert.match(html, /usAIge-0\.1\.12-alpha\.dmg\.sha256/);
...
test("publishes a checksum matching the current disk image", async () => {
  const dmg = await readFile(new URL("../public/usAIge-0.1.12-alpha.dmg", import.meta.url));
  const checksum = await readFile(
    new URL("../public/usAIge-0.1.12-alpha.dmg.sha256", import.meta.url),
    "utf8",
  );
  const digest = createHash("sha256").update(dmg).digest("hex");

  assert.equal(checksum.trim(), `${digest}  usAIge-0.1.12-alpha.dmg`);
});
...
test("publishes a valid automatic update manifest", async () => {
  ...
  assert.equal(
    ...
    "https://usaige-macos.richardqz.chatgpt.site/usAIge-0.1.12-alpha.dmg",
  );
});
