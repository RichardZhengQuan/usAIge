...
test("server-renders the current usAIge release", async () => {
  ...
  assert.match(
    html,
    /https:\/\/usaige-macos\.richardqz\.chatgpt\.site\/usAIge-0\.2\.8-alpha\.dmg/,
  );
  assert.match(
    html,
    /https:\/\/usaige-macos\.richardqz\.chatgpt\.site\/usAIge-0\.2\.8-alpha\.dmg\.sha256/,
  );
...
test("publishes a checksum matching the current disk image", async () => {
  const dmg = await readFile(new URL("../public/usAIge-0.2.8-alpha.dmg", import.meta.url));
  const checksum = await readFile(
    new URL("../public/usAIge-0.2.8-alpha.dmg.sha256", import.meta.url),
    "utf8",
  );
  const digest = createHash("sha256").update(dmg).digest("hex");

  assert.equal(checksum.trim(), `${digest}  usAIge-0.2.8-alpha.dmg`);
});
...
test("exports the same migration release for the legacy host", async () => {
  ...
  const dmg = await readFile(new URL("usAIge-0.2.8-alpha.dmg", legacyRoot));
  const digest = createHash("sha256").update(dmg).digest("hex");

  assert.match(
    html,
    /https:\/\/pmrichq\.com\/project\/usaige\/usAIge-0\.2\.8-alpha\.dmg/,
  );
  assert.deepEqual(
    { version: manifest.version, build: manifest.build, sha256: manifest.sha256 },
    { version: "0.2.8", build: 30, sha256: digest },
  );
  assert.equal(
    manifest.downloadURL,
    "https://pmrichq.com/project/usaige/usAIge-0.2.8-alpha.dmg",
