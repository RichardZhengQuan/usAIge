...
Download `usAIge-0.1.9-alpha.dmg` and its checksum from the website. Open the disk image, then drag `usAIge.app` onto the Applications shortcut.
...
(cd dist && shasum -a 256 -c usAIge-0.1.9-alpha.dmg.sha256)
```
